...
func DiskIOMetrics() (L []*model.MetricValue) {
	...
	for _, ds := range dsList {
		...
		L = append(L, CounterValueN("disk.io.read_requests", "读请求总次数", ds.ReadRequests, device))
		...
		L = append(L, CounterValueN("disk.io.msec_read", "所有读取花费的总ms数", ds.MsecRead, device))
		L = append(L, CounterValueN("disk.io.write_requests", "写请求总次数", ds.WriteRequests, device))
		...
		L = append(L, CounterValueN("disk.io.msec_write", "所有写入所花费的总ms数", ds.MsecWrite, device))
		L = append(L, CounterValueN("disk.io.ios_in_progress", "当前正在运行的实际IO请求数", ds.IosInProgress, device))
		L = append(L, CounterValueN("disk.io.msec_total", "总IO时间", ds.MsecTotal, device))
...
func IOStatsMetrics() (L []*model.MetricValue) {
	...
	for device := range diskStatsMap {
		...
		L = append(L, GaugeValueN("disk.io.read_bytes", "IO读总量", float64(delta_rsec)*512.0, tags))
		L = append(L, GaugeValueN("disk.io.write_bytes", "IO写总量", float64(delta_wsec)*512.0, tags))
		...
		L = append(L, GaugeValueN("disk.io.await", "每一个IO请求的处理的平均时间", await, tags))
		L = append(L, GaugeValueN("disk.io.svctm", "表示平均每次设备I/O操作的服务时间", svctm, tags))
		tmp := float64(use) * 100.0 / float64(duration)
		if tmp > 100.0 {
			tmp = 100.0
		}
		L = append(L, GaugeValueN("disk.io.util", "磁盘io利用百分比", tmp, tags))
	}
