...
func CpuMetrics() []*model.MetricValue {
	...
	cpuIdleVal := CpuIdle()
	idle := GaugeValueN("cpu.idle", "CPU空闲", cpuIdleVal)
	busy := GaugeValueN("cpu.busy", "CPU利用率", 100.0-cpuIdleVal)
	user := GaugeValueN("cpu.user", "CPU用户使用", CpuUser())
	system := GaugeValueN("cpu.system", "CPU系统使用", CpuSystem())
	iowait := GaugeValueN("cpu.iowait", "CPUIO等待时间", CpuIowait())
	...
	return []*model.MetricValue{idle, busy, user, system, iowait}
